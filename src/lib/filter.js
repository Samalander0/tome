  ...
  sites.map(site => {
    ...
    tags.map(tag => {
      if (!_.includes(site.tags, tag)) { // If the base site doesn't include the tag, set possible to false
        ...
        if (site.categories.pattern) { // Then, try looking through the categories on that site
          ...
          if (category_index !== -1) { // indexOf will return -1 if not found
            site.selected_category = {
              name: `${site.name} - ${tag}`,
              url: `${site.url}${site.categories.pattern}${_.kebabCase(site.categories.list[category_index])}`,
              image: site.image,
              tags: [...site.tags, tag],
            }
          } else {
